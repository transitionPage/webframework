...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		...
		DOM.close[config.showCloseIcon === false ? 'hide' : 'show']();
		...
		//按钮上方处的水平线显示控制，tip时无按钮隐藏
		if(!this.config.button || this.config.button.length==0) {
			jQuery(DOM.line[0]).css("display", "none");
		}
		else {
			jQuery(DOM.line[0]).css("display", "block");
		}
...
artDialog.defaults = {
	...
	minWidth: 96,				// 最小宽度限制
	minHeight: 32,				// 最小高度限制
	padding: '0px 25px 0px 0px',		// 内容与边界填充距离
	skin: '',					// 皮肤名(预留接口,尚未实现)
	icon: null,					// 消息图标名称
	time: null,					// 自动关闭时间
	esc: true,					// 是否支持Esc键关闭
	...
	left: '50%',				// X轴坐标
	top: '38.2%',				// Y轴坐标
	zIndex: 2002,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
	resize: true,				// 是否允许用户调节尺寸
	drag: true,					// 是否允许用户拖动位置
    showCloseIcon:true          // 标题栏上是否显示关闭按钮
};
